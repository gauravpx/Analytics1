...
filter(mtcars, cyl == 8) #only prints cyl value of 8
filter(mtcars, cyl < 6)
...
filter(mtcars, row_number() == n())     # n()- prints the last row
filter(mtcars, between(row_number(), 5, n()-2))  # problem that car name doesnt come
...
#sampling
sample_frac(mtcars, 0.2, replace=F)  # replace ensure same student not picked up
